...
{
    // Замок берём здесь, а не у вызывающего: сюда приходят и из задачи биения
    // (владелец отвязал машину в панели), а она про LVGL ничего не знает —
    // строить виджеты из неё без замка значит однажды словить порчу памяти.
    // Замок рекурсивный, поэтому вызовам из обработчиков событий он не мешает.
    display_lock(0);
    show_networks();
    display_unlock();
}
