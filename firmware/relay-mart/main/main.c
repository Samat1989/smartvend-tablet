...
// --- CONFIGURATION ---
// WiFi + machine identity are provisioned over a SoftAP setup portal and stored
// in NVS (same flow as esp-relay). Only the cloud endpoints stay fixed.
...
// Pins reverse-engineered from the stock SmartVend firmware (channel 0):
//   DIR = GPIO2 ("IO2") selects direction, PULSE = GPIO16 ("IO16") energizes the
//   coil for 50 ms; the relay then holds its state with no current.
#define CH0_DIR        GPIO_NUM_2
#define CH0_PULSE      GPIO_NUM_16
#define SETTLE_MS      100      // settle delay around the coil pulse
#define PULSE_MS       50       // coil energize pulse width
#define OPEN_SECONDS   20       // <-- how long to hold the relay open on a payment

// Re-provisioning trigger: tap the BOOT button (GPIO0) within the first
// PROVISION_WINDOW_MS AFTER power-on. (Sampling GPIO0 here, after the chip has
// booted, is safe — unlike holding it at reset, which enters ROM download mode.)
...
static void relay_pins_init(void) {
    gpio_config_t io = {
        .pin_bit_mask = (1ULL << CH0_DIR) | (1ULL << CH0_PULSE),
    ...
    gpio_config(&io);
    gpio_set_level(CH0_DIR, 0);
    gpio_set_level(CH0_PULSE, 0);
}

// Latching ACTIVATE: DIR HIGH, settle, pulse coil, settle, DIR LOW.
static void relay_on(void) {
    ESP_LOGI(TAG, "Relay ON: DIR(IO%d)=HIGH", CH0_DIR);
    gpio_set_level(CH0_DIR, 1);   vTaskDelay(pdMS_TO_TICKS(SETTLE_MS));
    gpio_set_level(CH0_PULSE, 1); vTaskDelay(pdMS_TO_TICKS(PULSE_MS));
    gpio_set_level(CH0_PULSE, 0); vTaskDelay(pdMS_TO_TICKS(SETTLE_MS));
    gpio_set_level(CH0_DIR, 0);
...
static void relay_off(void) {
    ESP_LOGI(TAG, "Relay OFF: DIR(IO%d)=LOW", CH0_DIR);
    gpio_set_level(CH0_DIR, 0);   vTaskDelay(pdMS_TO_TICKS(SETTLE_MS));
    gpio_set_level(CH0_PULSE, 1); vTaskDelay(pdMS_TO_TICKS(PULSE_MS));
    gpio_set_level(CH0_PULSE, 0);
    ESP_LOGI(TAG, "Relay cycle complete");
}
...
// Open the latching relay, hold OPEN_SECONDS, then close. Runs on its own task
// so the door timing never waits on the network.
static void trigger_lock(void *pvParameters) {
    is_lock_active = true;
    ESP_LOGI(TAG, "🔓 OPENING relay for %d s", OPEN_SECONDS);
    relay_on();
    vTaskDelay(pdMS_TO_TICKS(OPEN_SECONDS * 1000));
    relay_off();
...
// ============================ MQTT ============================
// Fires the door for a confirmed, non-duplicate order id.
static void handle_payment(const char *orderid) {
    if (is_lock_active) {
        ESP_LOGI(TAG, "Already opening... ignoring request.");
        return;
    }
    if (!orderid || !orderid[0]) return;
    if (strcmp(last_order_id, orderid) == 0) {
        ESP_LOGI(TAG, "Duplicate order ID detected (%s). Skipping.", last_order_id);
        return;
    }
    ESP_LOGI(TAG, "✅ New Payment confirmed! Order: %s", orderid);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        ...
        case MQTT_EVENT_DATA: {
            ...
            if (json) {
                // esp-relay payment event: {"msg":"Processed order","code":1,"orderid":...}
                ...
                if (msg && cJSON_IsString(msg) && strcmp(msg->valuestring, "Processed order") == 0 &&
                    ...
                    orderid && cJSON_IsString(orderid)) {
                    handle_payment(orderid->valuestring);
                }
                cJSON_Delete(json);
            }
            break;
...
static void wifi_event_handler(void* arg, esp_event_base_t base, int32_t id, void* data) {
    if (base == WIFI_EVENT && id == WIFI_EVENT_STA_START) {
        if (!g_provisioning) esp_wifi_connect();
...
// GET /device-provision?machid=<machid> -> { uuid, secret, name }.
...
static esp_err_t save_post_handler(httpd_req_t *req) {
    ...
    form_field(body, "ssid2", ssid2, sizeof(ssid2));
    form_field(body, "pass", pass, sizeof(pass));
    form_field(body, "machid", machid, sizeof(machid));
    if (ssid2[0]) strncpy(ssid, ssid2, sizeof(ssid) - 1);
...
// Watch BOOT (GPIO0) for the first `window_ms` after power-on. Returns true on a
// short press (~100 ms debounce — a brief tap is enough; no long hold needed).
static bool button_pressed_within(int window_ms) {
    gpio_config_t btn = {
    ...
    gpio_config(&btn);
    ESP_LOGI(TAG, "[Setup] tap BOOT within %d s to enter provisioning...", window_ms / 1000);
    int held = 0;
    for (int t = 0; t < window_ms; t += 50) {
        if (gpio_get_level(SETUP_BUTTON_GPIO) == 0) {   // BOOT pressed = low
            held += 50;
            if (held >= 100) {                          // ~100 ms debounce
                ESP_LOGW(TAG, "[Setup] BOOT pressed");
                return true;
            }
        } else {
            held = 0;
...
void app_main(void) {
    ...
    // Relay pins (latching IO2/IO16) — start in a known LOW state.
    relay_pins_init();
    ...
    // Tap BOOT within the first 4 s after power-on to (re)enter provisioning.
    // Checked on every boot, even when already configured.
    if (button_pressed_within(PROVISION_WINDOW_MS)) {
    ...
    wifi_start_normal();
    mqtt_start();
    ESP_LOGI(TAG, "Relay Mart Started (CH0 IO%d/IO%d, open %ds), machid=%s",
             CH0_DIR, CH0_PULSE, OPEN_SECONDS, g_machid);
}
