...
static const char *TAG = "pulse_mart";
...
// --- Firmware version + OTA (GitHub Releases, tag prefix "pulse-v") ---
// FW_VERSION_CODE is the monotonic number compared against the release tag.
// Bump both on every release (release-pulse.ps1 does this automatically).
// OTA shares the tablet's repo; firmware releases are tagged "pulse-vX.Y.Z" and
// carry an asset named OTA_ASSET_NAME, so they never collide with the APK — nor
// with the esp-relay stream, which owns the "relay-v*" tags and "relay-mart.bin".
// A board flashed with esp-pulse must never pick up an esp-relay build (its
...
// Lock output — a single line held at LOCK_ACTIVE_LEVEL for the provisioned
// time, then released. This is the one difference from esp-relay, which drives
// a bistable relay (DIR IO2 + coil pulse IO16); here IO2 IS the output and IO16
// is unused. Suits both an electric bolt lock whose "Open" wire is pulled to
// ground through a low-side switch and a plain monostable relay module.
//
// Active HIGH on purpose: GPIO2 is an ESP32 strapping pin and is not driven by
// firmware between reset and app_main, so the board must hold it low by itself
// (pulldown on the switch gate). "Not driven" then means "locked" across reset,
// bootloader and OTA. If the hardware turns out to be active LOW, fix the
// hardware — inverting this define would open the lock on every reboot.
#define LOCK_GPIO            GPIO_NUM_2    // IO2 — lock output
#define LOCK_ACTIVE_LEVEL    1             // 1 = held HIGH while open
#define DEFAULT_OPEN_SECONDS 20            // hold time if not provisioned
...
static int  g_open_seconds = DEFAULT_OPEN_SECONDS;  // lock hold time (provisioned)
...
// ============================ Lock output (held level) ============================
// Configure the lock line as an output and park it inactive. Safe to call once
// at boot — and it must be called early, because until it runs the pin is left
// to the board's own pulldown (see LOCK_GPIO).
static void lock_pins_init(void) {
    gpio_config_t io = {
...
// Drive the lock line active (door unlocked while held).
static void lock_open(void) {
    gpio_set_level(LOCK_GPIO, LOCK_ACTIVE_LEVEL);
}

// Release the lock line (door locked). Idempotent — also used as the boot-time
// safety park, though unlike the bistable relay of esp-relay this output cannot
// survive a reset on its own.
static void lock_close(void) {
    gpio_set_level(LOCK_GPIO, !LOCK_ACTIVE_LEVEL);
}

// Confirm the order with Supabase FIRST, then open the lock ONLY on HTTP 200.
...
// the lock stays closed and the order is NOT recorded, so an MQTT redelivery
...
static void handle_order_task(void *pvParameters) {
    ...
    if (confirmed) {
        ...
        ESP_LOGI(TAG, "🔓 LOCK OPEN (order %s confirmed 200, IO%d held %s) for %d s",
                 orderid, LOCK_GPIO, LOCK_ACTIVE_LEVEL ? "HIGH" : "LOW", g_open_seconds);
        lock_open();
        vTaskDelay(pdMS_TO_TICKS((uint32_t)g_open_seconds * 1000));
        lock_close();
        ESP_LOGI(TAG, "🔒 LOCK CLOSED");
    } else {
        ESP_LOGE(TAG, "❌ order %s NOT confirmed by server — lock stays CLOSED", orderid);
    }
...
// Parse a version code from a tag like "pulse-v1.2.3" or "pulse-v1.2.3+10203".
...
static char *ota_http_get(const char *url, int cap, int *out_status) {
    ...
    esp_http_client_set_header(cli, "User-Agent", "esp-pulse-ota");
...
// Find the newest "pulse-v*" release whose code beats ours and return its
// OTA_ASSET_NAME download URL. Two steps because the releases repo is SHARED
// with the tablet app and with esp-relay: the full /releases JSON (many verbose
// v1.1.x entries) overflowed our buffer, so the pulse-v* entries never parsed.
// Instead we pull the name-only /tags list (tiny) to pick the winning tag, then
// fetch just that one release for the asset URL — both fit in small buffers
// regardless of how many tablet/relay releases exist.
static bool ota_find_update(char *url_out, size_t url_sz) {
    // --- 1) newest pulse-v* tag from the lightweight /tags list ---
    char api[160];
    ...
    if (status == 200) {
        ...
        if (root && cJSON_IsArray(root)) {
            ...
            cJSON_ArrayForEach(t, root) {
                ...
                long code = ota_tag_code(nm->valuestring);   // -1 if not pulse-v*
                if (code > best) {
                    best = code;
    ...
    ESP_LOGI(TAG, "OTA: newest pulse tag %s (code %ld) > current %d",
             best_tag, best, FW_VERSION_CODE);

    // --- 2) fetch that release by tag for the .bin asset URL ---
    // URL-encode '+' in the tag (pulse-vX.Y.Z+CODE) as %2B for the path.
    char enc[72];
    int ei = 0;
...
// background so MQTT/lock stay responsive; reboots into new firmware if any.
...
static const char PAGE_TAIL_MID[] =
    "\">"
    "<span class=lbl>Время удержания замка, сек</span>"
...
void app_main(void) {
    ...
    // Lock line as an output, parked closed, as early as possible. The output is
    // level-held rather than latching, so a reset already drops it — but until
    // this runs the pin is only held by the board's pulldown, so the window is
    // shortened, not skipped.
    lock_pins_init();
    lock_close();
    ESP_LOGI(TAG, "[Boot] lock parked CLOSED (IO%d %s)",
             LOCK_GPIO, LOCK_ACTIVE_LEVEL ? "low" : "high");
