...
### Тип меняется, машина остаётся

`kind` — обычная колонка с CHECK, и суперадмин переключает её прямо в списке
флота: панель → «Пользователи» → «Все устройства», бейдж типа на карточке
машины стал селектором (`device-admin`, `action: 'kind'`).

До этого единственным способом было удалить машину и завести заново — вместе
со всей историей продаж и раскладкой. А тип меняется не от хорошей жизни:
холодильник переехал с планшета на статический QR, вендинговый шкаф стал
микромаркетом. Это перенастройка, а не новая машина.

Меняется только колонка. Продажи, `inventory` и привязка планшета не
трогаются: `kind` описывает, чем машина работает, а не что она продала. Уже
привязанный планшет продолжает работать — `kind` он читает только при
привязке (`verify_pairing`), а режим ему всё равно задаёт выбранный на месте
тип платы.

Единственная проверка на выходе — предупреждение при переводе в
`micromarket_screen`: там покупатель набирает номер ячейки, поэтому позиции
без `inventory.motor_id` или с номером вне 10–99 на экране автомата не
появятся. Панель называет их число, но смену не блокирует.
...
**В платёжном потоке бэкенд ничего не шлёт устройству.** Реле само инициирует
все обращения: событие об оплате приходит ему от брокера SmartVend, дальше оно
идёт в `complete-order` и открывает замок по 200.

Обратный канал у системы с тех пор появился, но он **вне этого потока** и
намеренно к нему не подключён: сервисное открытие замка для пополнения
(`service_opens` + свой MQTT-брокер, см.
[system_architecture.md](system_architecture.md#сервисное-открытие-замка-канал-панель--плата)).
Он не участвует ни в одном платеже и не пишет ни в `sales`, ни в
`pending_orders`.
...
2. **MQTT-брокер.** Для платежей свой не поднимали — используется брокер SmartVend `mqtt.smartvend.kz:14003`, auth по username/password (uuid/secret), топик `vending/{uuid}/in`. Обратного канала «бэкенд → реле» в платёжном потоке нет и не появилось. Свой брокер завели позже и для другого — сервисного открытия замка (топик `svc/{machid}/in`), потому что в чужой публиковать нельзя, а ронять деньги вместе со своим брокером не хотелось.
