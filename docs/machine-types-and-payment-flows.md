...
Документ описывает разделение системы на два **разных** типа продающих устройств и identity-модель, на которой они работают. Зафиксирован 2026-05-29 при проектировании static-QR флоу для customer_web.

> Этот документ заменяет устаревшую часть [system_architecture.md](../system_architecture.md), описывающую только POS-терминал. Старый файл оставлен на месте для исторической сверки, но не отражает текущее состояние.
...
|---|---|---|
| Аппаратура | M109E control board + моторы-спирали | ESP32 relay + электрозамок ([esp_relay_mart](../esp_relay_mart/)) |
| Приложение на машине | `m102_tester` (Android/Flutter) | **нет приложения** — только ESP32 |
...
| Кто инициирует платёж | Планшет m102_tester (вызывает LV API напрямую) | Edge-функция (вызывается с телефона покупателя по QR) |
...
| `micromarkets.kind` | `'vending'` | по плану `'static_qr'` (TBD — `'micromarket_tablet'` сейчас в legacy схеме) |
...
|---|---|---|---|
| **Owner** | `customer_web /admin` | Supabase Auth: `signInWithPassword(email, password)` | Видит свои машины (`owner_id = auth.uid()`), правит каталог/инвентарь/цены, читает продажи |
| **Kiosk** (только для vending) | `m102_tester` на планшете внутри машины | Supabase Auth: `signInWithPassword('kiosk-{machid}@local.smartvend', secret)` — один раз при пэйринге | Пишет `sales`/`sales_items` для **своей** машины (`kiosk_user_id = auth.uid()`), правит свой `inventory.stock` после выдачи |
| **Customer** (для static-QR) | Браузер телефона покупателя | **Никак** — anon role, без логина | Читает каталог. **В БД не пишет вообще** — все мутации происходят в edge-функциях с service_role после редиректа в Каспи |
...
```
Покупатель у машины (планшет m102_tester):
  • Выбирает товар на тач-экране
  • Tap "Оплатить" → m102_tester сам:
  ...
  • m102_tester polling'ит LV /payment_result каждые N сек
  • Когда LV отдаёт code=1:
      ...
      • m102_tester отчитывается обратно в LV: «выдано»
      • LV окончательно закрывает платёж
      • m102_tester пишет в Supabase: sales + sales_items + декремент stock
```

Здесь Supabase узнаёт о продаже **постфактум** через m102_tester (authenticated как kiosk-user). Бэкэнд не участвует в payment-флоу — он только хранит результат.
...
- Утечка `micromarkets.secret` через anon SELECT (E4/C1) закрывается column-grant'ом, потому что:
  - vending: m102_tester верифицирует секрет через `auth.signInWithPassword`, а не через прямой SELECT.
  - static-QR: секрет нужен только edge-функции, та работает с service_role.

См. полный аудит безопасности в [memory: project-supabase-security-state](../../.claude/projects/c--m109e/memory/project_supabase_security_state.md) — закроется ≥80% находок одной этой архитектурной перепрошивкой.
...
5. **`micromarkets.kind`** новое значение для static-QR — назвать `static_qr` или иначе, чтобы не путать с уходящими `'micromarket_tablet'`/`'kiosk'`.
...
- [memory: customer_web architecture](../../.claude/projects/c--m109e/memory/project_customer_web_architecture.md) — что уже умеет `customer_web` SPA
- [memory: m102_tester Supabase writes](../../.claude/projects/c--m109e/memory/project_m102_tester_supabase_writes.md) — инвентарь REST-запросов, которые делает vending-планшет
...
- [LE third-party QR payment API_V2.3.pdf](../LE%20third-party%20QR%20payment%20API_V2.3.pdf) — спецификация LV gateway
- [system_architecture.md](../system_architecture.md) — устаревший документ про старый POS-app
