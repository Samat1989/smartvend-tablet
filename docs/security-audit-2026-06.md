...
> **Статус на 2026-08-04.** Отчёт оставлен как исторический снимок и не
> переписывается. Что с тех пор изменилось: F1 и F2 закрыты миграциями
> `20260603120000`–`20260603170000` (secret не anon-читаем, anon-записи
> отозваны, планшет пишет через secret-scoped RPC). Функции
> `process_kiosk_sale` (F18), `verify-payment` (F4, F17) и
> `cron-process-payments` (F8, F17) **удалены из проекта** — соответствующие
> находки неактуальны. Актуальную картину платёжного потока смотрите в
