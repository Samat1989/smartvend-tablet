...
закреплено» и заблокированная шторка.
...
> Раньше здесь значились ещё «спрятанные системные полосы» и «перезагрузка
> устройства по команде». Ни то ни другое к device owner отношения не имеет.
> На планшетах SHENGMA полосы решает прошивка через свои property, а
> перезагрузку умеет любое приложение через вендорский broadcast — разбор в
> [05_SYSTEM_BARS_AND_AUTOSTART.md](05_SYSTEM_BARS_AND_AUTOSTART.md).
