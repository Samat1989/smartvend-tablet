# Release pipeline for the esp-pulse firmware (ESP32 OTA).
#
# Same idea as apps/tablet/scripts/release.ps1 (the tablet APK pipeline),
# but for the ESP-IDF firmware.
# Three release streams share the tablet's repo, kept apart purely by tag
# prefix + asset name: the tablet APK ("v*"), esp-relay ("relay-v*" +
# "relay-mart.bin") and esp-pulse ("pulse-v*" + "pulse-mart.bin"). The device's
# OTA checker (ota_find_update in main.c) only looks at its own prefix, so a
# pulse board never pulls a relay build and vice versa. Never point this script
# at firmware/esp-relay or reuse the relay asset name.
#
# What it does:
...
#   4. git-commits the bump, tags `pulse-vX.Y.Z+CODE`, pushes.
#   5. Creates a GitHub Release for that tag and uploads the .bin as
#      `pulse-mart.bin` - the asset name the device's OTA searches for.
#
# Usage (from an ESP-IDF PowerShell, so idf.py is on PATH):
#   .\release-pulse.ps1 "Add periodic OTA check"
#   .\release-pulse.ps1 -Message "..." -Version 1.2.0
#   .\release-pulse.ps1 -Message "..." -DryRun
#   .\release-pulse.ps1 -Message "..." -NoPush
...
$FwPath    = Join-Path $RepoRoot 'firmware/esp-pulse'
$MainC     = Join-Path $FwPath 'main/main.c'
$AssetName = 'pulse-mart.bin'                                   # OTA_ASSET_NAME in main.c
$BinPath   = Join-Path $FwPath 'build/esp_pulse_mart.bin'       # project(esp_pulse_mart)
